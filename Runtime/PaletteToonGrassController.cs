...
[ExecuteAlways]
public class PaletteToonGrassController : MonoBehaviour
{
    ...
    [Header("Setup")]
    [Tooltip("The grass material asset to configure. Shared across all terrain detail instances.")]
    ...
    public void Apply()
    {
        ...
        {
            Debug.LogWarning("PaletteToonGrassController: failed to read palette — " + e.Message, this);
        }
        ...
        grassMaterial.SetColor(ColorShadowId,     GetCachedColor(shadowColorIndex));
        grassMaterial.SetColor(ColorBaseId,       GetCachedColor(baseColorIndex));
        grassMaterial.SetColor(ColorHighlightId,  GetCachedColor(highlightColorIndex));
        grassMaterial.SetColor(BaseColorId,       baseTint);
        grassMaterial.SetFloat(Threshold1Id,      shadowThreshold);
        grassMaterial.SetFloat(Threshold2Id,      highlightThreshold);
        grassMaterial.SetFloat(IntensityAffectsBandsId, intensityAffectsBands);
        grassMaterial.SetFloat(BandAccumulationId, (float)bandAccumulation);
        grassMaterial.SetFloat(ApplyFogId, applyFog ? 1f : 0f);
    }

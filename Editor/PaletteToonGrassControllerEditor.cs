...
[CustomEditor(typeof(PaletteToonGrassController))]
public class PaletteToonGrassControllerEditor : Editor
{
    ...
    private SerializedProperty _grassMaterial;
    ...
    private const string AdvancedFoldoutKey = "PaletteToonGrass_ShowAdvanced";

    private void OnEnable()
    {
        _grassMaterial = serializedObject.FindProperty("grassMaterial");
    ...
    public override void OnInspectorGUI()
    {
        ...
        PaletteToonGrassController ctrl = (PaletteToonGrassController)target;

        // ── Setup ──
        EditorGUILayout.PropertyField(_grassMaterial,
            new GUIContent("Grass Material",
                "The grass material asset to configure. Shared across all terrain detail instances."));
        EditorGUILayout.PropertyField(_paletteTexture);

        // Warn if material shader doesn't match
        if (ctrl.grassMaterial != null && ctrl.grassMaterial.shader != null &&
            ctrl.grassMaterial.shader.name != "Custom/PaletteToonGrass")
        {
            EditorGUILayout.HelpBox(
                $"Material uses shader '{ctrl.grassMaterial.shader.name}'. " +
                "Expected 'Custom/PaletteToonGrass'.",
                MessageType.Warning);
        }
